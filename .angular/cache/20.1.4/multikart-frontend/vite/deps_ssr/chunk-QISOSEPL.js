...
var ɵNgxsAppBootstrappedState = class _ɵNgxsAppBootstrappedState extends import_rxjs.BehaviorSubject {
  constructor() {
    super(false);
...
var InternalActions = class _InternalActions extends ɵOrderedSubject {
  ...
  dispatched$ = new import_rxjs2.Subject();
  constructor() {
    super();
    this.subscribe((ctx) => {
...
var Actions = class _Actions extends import_rxjs2.Observable {
  constructor() {
    ...
    const sharedInternalActions$ = new import_rxjs2.Subject();
...
var InternalDispatcher = class _InternalDispatcher {
  ...
  dispatchByEvents(actionOrActions) {
    if (Array.isArray(actionOrActions)) {
      ...
      return (0, import_rxjs2.forkJoin)(actionOrActions.map((action) => this.dispatchSingle(action))).pipe((0, import_rxjs2.map)(() => void 0));
    } else {
  ...
  dispatchSingle(action) {
    if (typeof ngDevMode !== "undefined" && ngDevMode) {
      ...
      if (!type) {
        ...
        return new import_rxjs2.Observable((subscriber) => subscriber.error(error));
      }
    }
    ...
    }])(prevState, action).pipe((0, import_rxjs2.shareReplay)());
  }
  getActionResultStream(action) {
    return this._actionResults.pipe((0, import_rxjs2.filter)((ctx) => ctx.action === action && ctx.status !== ActionStatus.Dispatched), (0, import_rxjs2.take)(1), (0, import_rxjs2.shareReplay)());
  }
  createDispatchObservable(actionResult$) {
    return actionResult$.pipe((0, import_rxjs2.mergeMap)((ctx) => {
      switch (ctx.status) {
...
function filterStatus(allowedTypes, allowedStatuses) {
  return (0, import_rxjs2.filter)((ctx) => {
...
function mapAction() {
  return (0, import_rxjs2.map)((ctx) => ctx.action);
}
  ...
  createActionHandler(path, handlerFn, options) {
    ...
    return (action) => {
      ...
      if ((0, import_rxjs2.isObservable)(result)) {
        result = result.pipe(
          (0, import_rxjs2.mergeMap)((value) => isPromise(value) || (0, import_rxjs2.isObservable)(value) ? value : (0, import_rxjs2.of)(value)),
        ...
        if (options.cancelUncompleted) {
          ...
          result = result.pipe((0, import_rxjs2.takeUntil)(canceled));
        }
        result = result.pipe(
...
var StateFactory = class _StateFactory {
  ...
  connectActionHandlers() {
    this._actionsSubscription = this._actions.pipe((0, import_rxjs2.filter)((ctx) => ctx.status === ActionStatus.Dispatched), (0, import_rxjs2.mergeMap)((ctx) => {
    ...
    if (actionHandlers) {
      for (const actionHandler of actionHandlers) {
        ...
        try {
          ...
          result = new import_rxjs2.Observable((subscriber) => subscriber.error(e));
        }
        results.push(result);
...
var Store = class _Store {
   ...
   */
  _selectableStateStream = this._stateStream.pipe(leaveNgxs(this._internalExecutionStrategy), (0, import_rxjs2.shareReplay)({
    bufferSize: 1,
    refCount: true
  }));
  constructor() {
...
var LifecycleStateManager = class _LifecycleStateManager {
  ...
  ngxsBootstrap(action, results) {
    ...
    this._internalStateOperations.getRootStateOperations().dispatch(action).pipe((0, import_rxjs2.mergeMap)(() => {
      if (!results) {
        return import_rxjs2.EMPTY;
      }
    ...
    for (const mappedStore of mappedStores) {
      ...
      if (instance.ngxsOnChanges) {
        ...
        this._store.select((state) => getValue(state, mappedStore.path)).pipe(
          ...
          (0, import_rxjs2.startWith)(void 0),
          // `skip` is using `filter` internally.
          (0, import_rxjs2.skip)(1)
...
function withNgxsPendingTasks() {
  return withNgxsPreboot(() => {
    ...
    const subscription = actions$.pipe(
      ...
      (0, import_rxjs2.buffer)(actions$.pipe((0, import_rxjs2.debounceTime)(0)))
    ).subscribe((contexts) => {
...
//# sourceMappingURL=chunk-QISOSEPL.js.map
